...
export class operationMap{
    ...
    EchoDrawing(data) {
        data.forEach((item) => {
          ...
          let feature = new CTMapOl.DataSourceControl.common.addSingleGeometryDataSource(
              {
                mapRef: this.mapRef,
    ...
    creatWMSLayer(params,year){
      ...
      this.wmsLayerObj=new CTMapOl.LayerControl.lib.WMSLayer(
        ...
        },
        {
          ...
          onSelectFunc:(map, event, data, coord) => {
            ...
            this.changeWindow('detailsPopupShow',true)
            this.setMapZoom(11,coord)
          },
          onUnselectFunc: (map, event, data) => {
            this.changeWindow('detailsPopupShow',false)
          }
        }
      )
      ...
      this.changeWindow("detailsPopupShow", false);
      this.changeWindow('legendShow',true)
    }
    ...
    clearWmsLayer(){
      ...
      this.changeWindow('legendShow',false)
      this.changeWindow('detailsPopupShow',false)
      window.newMapClass = null
    }

    //改变window自定义变量的值并通知
    changeWindow(type,status){
      window[type]=status
      window.dispatchEvent(new Event('detailsPopupShowChange'))
    }
}