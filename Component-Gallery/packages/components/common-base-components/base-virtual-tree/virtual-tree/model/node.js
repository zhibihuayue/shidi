...
export default class Node {
  ...
  // prettier-ignore
  insertChild(child, index, batch) { // NOSONAR
    ...
    if (!(child instanceof Node)) {
      if (!batch) {
        ...
        if (children.indexOf(child.data) === -1) {
          // prettier-ignore
          if (typeof index === 'undefined' || index < 0) { // NOSONAR
      ...
      this.loadData((data) => {
        if (data instanceof Array) {
          if (this.checked) {
            ...
            this.expandDone(callback, expandParent)
            return
          }
  ...
  expandDone(callback, expandParent) {
    if (expandParent) {
  ...
  // prettier-ignore
  setChecked(value, deep, recursion, passValue) { // NOSONAR
  ...
  loadData(callback, defaultProps = {}) {
    ...
    if (this.store.lazy === true && loaded) {
      this.loading = true
