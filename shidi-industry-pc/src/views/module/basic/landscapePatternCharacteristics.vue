...
<template>
    <div class="landscapePatternCharacteristics">
        <basis-box name="景观格局特征">
            <div class="mainContent">
               <div class="popPicker">
                    <yearMonthDayTime :selectShow="false" :circleClose="false" timeType="year" moduleType="features" :dateRangeProp="defaultTime" @changeTime="changeTime" :haveDataTime="haveDataTime" :key="timekey"/>
                </div>
                <div class="tableArea">
                    <tableIndex :tableData="tableData" :tableHeadList="tableHeadList" />
                </div>
            </div>
        </basis-box>
    </div>
...
import { statisticsDefaultTime, statisticsFeature } from '@/http/environment.js'
...
export default {
    ...
    data(){
        return {
            tableData: [],
            tableHeadList: [
                { label: '生态系统格局', prop: 'name',align:"left",width:108},
                { label: '斑块数(个)', prop: 'plaqueCount',align:"left",width:108 },
                { label: '总面积(公顷)', prop: 'allArea',align:"left",width:108},
                { label: '平均斑块面积(公顷)', prop: 'avgArea',align:"left",width:108}
            ],
            nameMap: new Map([
                [1, '河流湿地'],
                [2, '湖泊湿地'],
  ...
  methods: {
    ...
    getDefaultTime(res) {
        if (res.code === 200) {
            ...
            this.timeList = this.defaultTime
            sessionStorage.setItem('date_featuresTime', JSON.stringify(this.defaultTime))
            this.getStatisticsFeature()
        } else {
            this.defaultTime = ''
        }
