...
<template>
  <div class="climatic">
    <basis-box :name="componentName" isHandShowKey="ClimateStatistics">
      <div class="staticMain" :style="{ height: pxToRem(commonentHeight) }">
        <!-- 顶部月年数据 -->
        <div class="topStatic">
          <ul class="topMain">
            <li v-for="item in analyseList" :key="item.type" class="mainList">
              ...
              <div class="text-wrap">
                ...
                <div class="num"
                  >{{ item.value }}<span>{{ item.unit }}</span></div
                >
              </div>
            </li>
          </ul>
        ...
        <div class="echatBox">
          <div class="legend-wrap">
            <div class="img-icon">
              <img
                src="@component-gallery/climateStatistics/assets/image/rain.svg"
                alt="降雨"
              />
              <span>降水量</span>
            </div>
            <div class="img-icon">
              <img
                src="@component-gallery/climateStatistics/assets/image/temperature.svg"
                alt="温度"
...
import { doubleFoline3D } from '../utils/index.js'
import { climaticAnalyst } from '../service/index.js'
...
export default {
  ...
  data() {
    return {
      ...
      // 选择的类型
      selectType:
        JSON.parse(sessionStorage.getItem('date_climaticTimeType')) || 'year', // 年月日自定义
      getTimeMap: new Map([
  ...
  methods: {
    // 初始化时间
    initTime() {
      this.selectType = JSON.parse(
        sessionStorage.getItem('date_climaticTimeType')
      )
      // 有表示为有时间记忆，无---默认为年
      if (this.selectType) {
        const currentHistoryTime = this.capitalizeFirstLetter(this.selectType)
        this.defaultTime = JSON.parse(
          sessionStorage.getItem(currentHistoryTime)
        )
      } else {
        this.selectType = 'year'
        sessionStorage.setItem(
          'date_climaticTimeType',
          JSON.stringify(this.selectType)
        )
        sessionStorage.setItem(
          'date_climaticHistoryTimeYear',
    ...
    getEcharts(XData, YData1, YData2) {
      ...
      this.myChart = echarts.init(this.$refs.histogramMap)
      const times =
        typeof this.defaultTime === 'string'
          ? this.defaultTime
          : moment(this.defaultTime[0]).format('YYYY-MM')
      this.myChart.setOption(
        doubleFoline3D(XData, YData1, YData2, times, this.selectType)
      )
    },
    /**
     * @description 气候分析统计
    ...
    async getClimaticAnalyst() {
      ...
      if (res.code === 200) {
        ...
        this.analyseList.forEach((element) => {
          element.value =
            element.keyType === 'temperatureMonth'
              ? Math.round(resData[element.keyType] * 10) / 10
              : Math.round(resData[element.keyType])
        })
        // 柱状图的xdata的值
        ...
        // 降水数据
        this.precipitationList = resData.precipitationList.map(
          (item) => item.precipitation
        )
        // 温度数据
        this.temperatureList = resData.temperatureList.map(
  ...
  .topStatic {
    ...
    .topMain {
      ...
      .mainList {
        ...
        .nameMouth {
          background: url('~@component-gallery/climateStatistics/assets/image/bird7.png')
            no-repeat;
          background-size: 100% 100%;
        }
        .nameYear {
          background: url('~@component-gallery/climateStatistics/assets/image/bird8.png')
            no-repeat;
          background-size: 100% 100%;
        }
        .text-wrap {
        ...
        &:nth-child(1) {
          background: url('~@component-gallery/climateStatistics/assets/image/bird9.png')
            no-repeat;
          background-size: 100% 100%;
          .text-wrap {
            .title {
        ...
        &:nth-child(2) {
          background: url('~@component-gallery/climateStatistics/assets/image/bird10.png')
            no-repeat;
          background-size: 100% 100%;
          .text-wrap {
            .title {
