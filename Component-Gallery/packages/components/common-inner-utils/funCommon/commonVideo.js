...
import CommonMessage from './message/common-message'
...
export class $v {
  ...
  static delete(vue, url, params, success, error, loadParam = {}) {
    loadParam.connectMode = 'delete'
    this.beforeConnect(vue, url, params, success, error, loadParam)
  }
  ...
  static connect(vue, url, params, success, error, loadParam) {
    ...
    if ('post' === connectMode) {
      ...
      if (loadParam.isLocation) {
        ...
        this.requestSDK(vue, url, params, 'delete').then(
          function (d) {
            //关闭加载动画
            _this.closeLoading(loading, loadingInstance)
            if (d.code === 200) {
...
export function dragBind(el, iframe, unstop) {
  el.onmousedown = (e1) => {
    if (!unstop) {
      e1.stopPropagation()
    }
...
export const urlSystem = 'VIDEO-APPLY-SYSTEM' //system
