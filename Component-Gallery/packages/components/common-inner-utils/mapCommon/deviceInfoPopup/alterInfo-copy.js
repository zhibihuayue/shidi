...
// forestry-pc/src/components/ComFooterFunsArea/components/deviceInfoPopup/alterInfo.js
import CommonMap from '../../mapCommon/mapTool/CommonMap'
import { cameraLocationAnalysisNew } from '../../mapCommon/map-ol/CommonCtMapOl'
...
import {
  ...
  queryHornListForSite, //大喇叭点位列表
  getDeviceVisibleBySite,
  queryIotDeviceBySite, //物联设备
  getUavDevicesBySiteCode, //无人机点位列表
  forestryQryUavDeviceInfo,
  getCameraList,
  forestryQueryDeviceForWE,
  getDeviceImage,
  gridKeeper_byid, //网格员详情弹窗
  getResourceInfo_byResourceId, //资源详情
  getTrapListByLatLon, //诱捕器点位列表
  getTrapInfoById, //诱捕器详情
  getRecordListofTrapByLatLon, //记录点位列表
  queryTrapGatherInfoById,
  getTreesList, //获取古树列表
  getAncientTreeDetails, //获取古树详情弹框
  getPreciousTreeDetails //获取名木详情弹框
...
export default {
  ...
  getmapid(is3d, isHaveWarningList, instance, mapContainerId, supview) {
    ...
    // console.log('获取mapid',deviceWindowsId,instance);
  },
  ...
  ExportCloseDeviceWindow(
    ...
    if (this._supview) {
      if (id == 7) {
        // if (this._supview.$refs.leftMonitorInfo.$refs.monitorTreeGridMan) {
        this._supview.$refs.leftMonitorInfo.$refs.monitorTreeGridMan?.setCurrentKey()
        // }
      } else if (id == 8) {
        // if (this._supview.$refs.leftMonitorInfo.$refs.monitorTreeZy) {
        this._supview.$refs.leftMonitorInfo.$refs.monitorTreeZy?.setCurrentKey()
        // }
      } else {
  ...
  // 处理打点的点击事件
  _pointClickHandle(dInfo, e, max = false) {
  ...
  return _arr
    .map((item, index) => {
      if (item.deviceStatus === 0 || item.status === 0) {
        return deviceStatusequaltoZero(item, index, _obj, id, _arr)
      } else if (id === 20) {
        console.log('诱捕器状态', item.deploymentSituation)
        return deploymentSituationtwenty(item, index, _obj, id, _arr)
      } else {
        return getdeviceListelseHtml(item, index, _obj, id, _arr)
      }
    })
    .join('')
})()}
...
<div class="cameraInfoBox"></div>
      ...
      _fn(param).then((res) => {
        if (res.code == '200') {
          ...
          if (_arr.length > 1) {
            ...
            this._setdeviceListHtml(_arr, deviceInfo, mapintance, openSucCblock)
          } else {
            // =============================
  ...
  _handleOtherDeviceType(
    ...
    if (id == 1) {
      ...
      return this.getCameraList(deviceInfo, mapintance, myCallSuscessBlock)
    }
    // 采集记录单独处理
    if (id == 21) {
      return this.getRecordListTrapList(
        deviceInfo,
        mapintance,
        myCallSuscessBlock
      )
    ...
    // 小班单独处理
    if (id == 22) {
      return this.smallClassDetailWindow(deviceInfo, mapintance, openSucCblock)
    }
    // 古树名木列表单独处理
    if (id == 23) {
      this.getDict()
  ...
  //小班详情弹窗
  smallClassDetailWindow(info, mapintance, infoBlock) {
    console.log('小班弹窗info', info)
...
<div class="device-CameraListinfo-Window">
              ...
              return _arr
                .map((item, index) => {
                  ...
                  } InfoWindowCameraItem_${index}" data-length="${
                    _arr.length
                  }" data-id="21" data-index="${index}">
								<i class='iconfont_tools icon-linye_icon_caijijilu'></i>
                                <a class="span_text InfoWindowCameraItem  InfoWindowCameraItem_${index}"  data-length="${
                    _arr.length
                  }" data-id="21" style="flex-grow: 1;" data-index="${index}" title="${
              ...
              return _arr
                .map((item, index) => {
                  return `<li class="InfoWindowCameraItem  InfoWindowCameraItem_${index}" data-length="${
                    _arr.length
                  }" data-id="1" data-index="${index}" title="${
                    item.deviceName
                  }">
...
`
  ...
  // 获取摄像机点位列表
  getCameraList(deviceInfo, mapintance, infoBlock) {
    let paramData = {}
    if (deviceInfo.params) {
      paramData = deviceInfo.params
    }
    paramData.siteCode = deviceInfo.siteCode
      ? deviceInfo.siteCode
      : deviceInfo.code
    // code---站点code
    // siteCode -- 站点code
    console.log('摄像机点位列表入参', paramData)
    getCameraList(paramData).then((res) => {
      if (res.code == '200') {
        // this._isClickMark = false;
  ...
  handleCameraListViewShed(cameraList, siteCode, deviceCode) {
    ...
    Promise.all(reqList).then((resList) => {
      console.log(resList, cameraList)
  ...
  _getTreeLevelLabel(data) {
    let levelLabel = ''
    if (data.treeGrade == 1) {
      levelLabel = '<span class="" style="margin-left: 10px;">一级古树</span>'
    } else if (data.treeGrade == 2) {
      levelLabel =
        '<span class="" style="background: linear-gradient(90deg, #F56C6C 0%, #DA2727 100%);margin-left: 10px;">二级古树</span>'
    } else if (data.treeGrade == 3) {
      levelLabel =
        '<span class="" style="background: linear-gradient(90deg, #52A1E5 0%, #1F7CCC 100%);margin-left: 10px;">三级古树</span>'
    } else {
      levelLabel =
        '<span class="" style="background: linear-gradient(135deg, #FFA43C 0%, #FF8D0B 100%);margin-left: 10px;">名木</span>'
    }
    return levelLabel
  },
  //古树名木详情弹窗
...
<div class="el-dialog__header ">
  <div  class=" name" style="flex-direction: row;display: flex;width: 335px;">
   ...
   <span class="${
     _obj.status == 1 ? 'offline' : ''
   }" style="margin-left: 10px;">${_obj.status == 1 ? '离线' : '在线'}</span>
...
<div class="dotContent">
  ...
  _setSmalClassDetailInfoWindowsHtml(id, data, type = 1) {
    ...
    // dotBox deviceWindowsIdviceWindowsId attrDotDetailsBox
    // dialogBox dotBox  weichuangwindowdeviceInfoLy deviceWindowsIdviceWindowsId attrDotDetailsBox
    // _setMaekHtml
    // ${type == 1 ? tempListClass : tempClass}"""
    let endstr = `<div style="transform: scale(${this.scaleY})" ref="deviceWindowsId" class="dotBox deviceWindowsIdviceWindowsId attrDotDetailsBox" data-code="${data.code}">
	 <i data-id="${id}" class="closeImg InfoWindowCloseBtn"></i>
	 <div class="el-dialog__header ">
	 <div  class=" name" style="flex-direction: row;display: flex;">
  ...
  _setMaekHtml(id, data, type = 1) {
    ...
    let endstr = `<div style="${
      type == 2 ? `transform: scale(${this.scaleY});` : ''
    }" ref="deviceWindowsId" class="dotBox deviceWindowsIdviceWindowsId attrDotDetailsBox" data-code="${
      data.deviceCode
    }">
 <i data-id="${id}" class="closeImg InfoWindowCloseBtn"></i>
 <div class="el-dialog__header ">
 ...
 <div class="dotContent">
  ...
  _setResourceManMaekHtml(id, data, type = 1) {
    ...
    return `<div style="transform: scale(${
      this.scaleY
    });" class="resourceDetailsPopUp dotBox deviceWindowsIdviceWindowsId attrDotDetailsBox" data-code="${
      data.deviceCode
    }">
  ...
  _setGridManMaekHtml(id, data, type = 1) {
    ...
    let GridManMaekHtmlStr = `<div style="transform: scale(${
      this.scaleY
    });" class="keeperDetailBox dotBox deviceWindowsIdviceWindowsId attrDotDetailsBox" data-code="${
      data.deviceCode
    }">
		<i data-id="${id}" class="closeImg ${
      type == '1' ? '' : 'InfoWindowCloseBtn'
  ...
  // 切换可视域
  _switchviewShed(dom) {
    console.log('dom.dataset.code:', dom.dataset.code)
    //设备离线不展示可视域
    ...
    if (this.is3Dprivate) {
      ...
      this.No3DprivateswitchviewShed(dom, _flag)
    }
  },
  No3DprivateswitchviewShed(dom, _flag) {
